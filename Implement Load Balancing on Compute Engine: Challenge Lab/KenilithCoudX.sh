...
echo "${RED_TEXT}${BOLD_TEXT}Detecting Zone & Region...${RESET_FORMAT}"

ZONE=$(gcloud compute project-info describe \
  --format="value(commonInstanceMetadata.items[google-compute-default-zone])")

if [ -z "$ZONE" ]; then
    echo "${RED_TEXT}${BOLD_TEXT}Default zone not found. Enter zone (us-east1-d):${RESET_FORMAT}"
    read -p "Zone: " ZONE
fi
...
echo "${BLUE_TEXT}${BOLD_TEXT}Using Zone: $ZONE${RESET_FORMAT}"
echo "${BLUE_TEXT}${BOLD_TEXT}Using Region: $REGION${RESET_FORMAT}"
echo
echo "${RED_TEXT}${BOLD_TEXT}Tech & Code - https://www.youtube.com/@TechCode9${RESET_FORMAT}"
echo

# ---------------- Task 1: Create Web Servers -------------------
echo "${RED_TEXT}${BOLD_TEXT}Creating web1, web2, web3...${RESET_FORMAT}"
...
echo
echo "${BLUE_TEXT}${BOLD_TEXT}Web servers created successfully.${RESET_FORMAT}"
echo

# Firewall for Network LB
echo "${RED_TEXT}${BOLD_TEXT}Creating firewall rule www-firewall-network-lb...${RESET_FORMAT}"
...
echo "${BLUE_TEXT}${BOLD_TEXT}Firewall rule created.${RESET_FORMAT}"
echo

# ---------------- Task 2: Network Load Balancer -------------------
echo "${RED_TEXT}${BOLD_TEXT}Configuring Network Load Balancer...${RESET_FORMAT}"
...
echo "${BLUE_TEXT}${BOLD_TEXT}Network Load Balancer created. IP: $NLB_IP ${RESET_FORMAT}"
echo
echo "${RED_TEXT}${BOLD_TEXT}Tech & Code - https://www.youtube.com/@TechCode9${RESET_FORMAT}"
echo

# ---------------- Task 3: HTTP Load Balancer -------------------
echo "${RED_TEXT}${BOLD_TEXT}Setting up HTTP Load Balancer...${RESET_FORMAT}"
