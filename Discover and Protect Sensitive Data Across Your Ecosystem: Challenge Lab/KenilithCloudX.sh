...
# Step 7: Create SPII Tag Key (skip if it already exists)
echo "${YELLOW}${BOLD}Creating resource tag key for SPII${RESET}"
EXISTING_KEY_ID=$(gcloud resource-manager tags keys list --parent="projects/${PROJECT_NUMBER}" --format="value(NAME)" --filter="shortName=SPII" 2>/dev/null)
...
if [[ -z "$EXISTING_KEY_ID" ]]; then
    ...
    echo "Tag key SPII already exists, skipping creation."
fi

# Step 8: Retrieve Tag Key ID
echo "${CYAN}${BOLD}Step 9: Retrieving tag key ID${RESET}"
TAG_KEY_ID=$(gcloud resource-manager tags keys list --parent="projects/${PROJECT_NUMBER}" --format="value(NAME)" --filter="shortName=SPII")

# Step 9: Create Tag Values (skip any that already exist)
echo "${MAGENTA}${BOLD}Creating tag values for Yes/No SPII${RESET}"

EXISTING_YES=$(gcloud resource-manager tags values list --parent="$TAG_KEY_ID" --format="value(shortName)" --filter="shortName=Yes" 2>/dev/null)
if [[ -z "$EXISTING_YES" ]]; then
    gcloud resource-manager tags values create Yes \
        --parent=$TAG_KEY_ID \
        --description="Contains sensitive personally identifiable information (SPII)"
else
    echo "Tag value Yes already exists, skipping creation."
fi

EXISTING_NO=$(gcloud resource-manager tags values list --parent="$TAG_KEY_ID" --format="value(shortName)" --filter="shortName=No" 2>/dev/null)
if [[ -z "$EXISTING_NO" ]]; then
    ...
    echo "Tag value No already exists, skipping creation."
fi
