...
echo "${BLUE_TEXT}Project ID set to: ${BOLD_TEXT}$PROJECT_ID${RESET_FORMAT}"
echo "${BLUE_TEXT}Project Number set to: ${BOLD_TEXT}$PROJECT_NUMBER${RESET_FORMAT}"
echo

echo "${BLUE_TEXT}${BOLD_TEXT}🛠️ Enabling the OS Config API for your project...${RESET_FORMAT}"
gcloud services enable osconfig.googleapis.com
echo

echo "${BLUE_TEXT}${BOLD_TEXT}🛡️ Creating firewall rule to allow HTTP traffic...${RESET_FORMAT}"
...
echo "${BLUE_TEXT}${BOLD_TEXT}🛡️ Creating firewall rule for health checks...${RESET_FORMAT}"
...
echo "${BLUE_TEXT}${BOLD_TEXT}📄 Creating instance template for REGION1: ${REGION1}...${RESET_FORMAT}"
...
echo "${BLUE_TEXT}${BOLD_TEXT}📄 Creating instance template for REGION2: ${REGION2}...${RESET_FORMAT}"
...
echo "${BLUE_TEXT}${BOLD_TEXT}🏗️ Creating Managed Instance Group (MIG) and configuring autoscaling for REGION1: ${REGION1}...${RESET_FORMAT}"
...
echo "${BLUE_TEXT}${BOLD_TEXT}🏗️ Creating Managed Instance Group (MIG) and configuring autoscaling for REGION2: ${REGION2}...${RESET_FORMAT}"
...
echo "${BLUE_TEXT}${BOLD_TEXT}🩺 Creating a global HTTP health check via API...${RESET_FORMAT}"
...
echo
echo "${RED_TEXT}Waiting for health check to be provisioned... ⏳${RESET_FORMAT}"
for i in $(seq 30 -1 1); do
  echo -ne "${RED_TEXT}  $i seconds remaining... \r${RESET_FORMAT}"
  sleep 1
done
echo -e "\n"

echo "${BLUE_TEXT}${BOLD_TEXT}🛡️ Creating a default security policy for the backend service via API...${RESET_FORMAT}"
curl -X POST -H "Content-Type: application/json" \
  -H "Authorization: Bearer $token" \
  -d '{
...
echo
echo "${RED_TEXT}Waiting for security policy to be provisioned... ⏳${RESET_FORMAT}"
for i in $(seq 30 -1 1); do
  echo -ne "${RED_TEXT}  $i seconds remaining... \r${RESET_FORMAT}"
  sleep 1
done
...
echo "${BLUE_TEXT}${BOLD_TEXT}⚙️ Creating the global backend service via API...${RESET_FORMAT}"
curl -X POST -H "Content-Type: application/json" \
  -H "Authorization: Bearer $token" \
  -d '{
    "backends": [
...
echo
echo "${RED_TEXT}Waiting for backend service to be provisioned... ⏳${RESET_FORMAT}"
...
echo "${BLUE_TEXT}${BOLD_TEXT}🗺️ Creating the URL map for the load balancer via API...${RESET_FORMAT}"
curl -X POST -H "Content-Type: application/json" \
  -H "Authorization: Bearer $token" \
  -d '{
...
echo
echo "${RED_TEXT}Waiting for URL map to be provisioned... ⏳${RESET_FORMAT}"
for i in $(seq 30 -1 1); do
  echo -ne "${RED_TEXT}  $i seconds remaining... \r${RESET_FORMAT}"
  sleep 1
done
...
echo "${BLUE_TEXT}${BOLD_TEXT}🎯 Creating the first Target HTTP Proxy via API...${RESET_FORMAT}"
curl -X POST -H "Content-Type: application/json" \
  -H "Authorization: Bearer $token" \
  -d '{
...
echo "${BLUE_TEXT}${BOLD_TEXT}🎯 Creating the second Target HTTP Proxy via API...${RESET_FORMAT}"
curl -X POST -H "Content-Type: application/json" \
  -H "Authorization: Bearer $token" \
  -d '{
  ...
  "https://compute.googleapis.com/compute/v1/projects/$project_id/global/targetHttpProxies"
echo
echo "${RED_TEXT}Waiting for target proxies to be provisioned... ⏳${RESET_FORMAT}"
for i in $(seq 30 -1 1); do
  echo -ne "${RED_TEXT}  $i seconds remaining... \r${RESET_FORMAT}"
  sleep 1
done
...
echo "${BLUE_TEXT}${BOLD_TEXT}➡️ Creating the IPv4 Global Forwarding Rule via API...${RESET_FORMAT}"
curl -X POST -H "Content-Type: application/json" \
  -H "Authorization: Bearer $token" \
  -d '{
    "IPProtocol": "TCP",
...
echo
echo "${RED_TEXT}Waiting for forwarding rule to be provisioned... ⏳${RESET_FORMAT}"
...
echo "${BLUE_TEXT}${BOLD_TEXT}➡️ Creating the IPv6 Global Forwarding Rule via API...${RESET_FORMAT}"
curl -X POST -H "Content-Type: application/json" \
  -H "Authorization: Bearer $token" \
  -d '{
    "IPProtocol": "TCP",
...
echo "${BLUE_TEXT}${BOLD_TEXT}🏷️ Setting named ports for the instance group in REGION1: ${REGION1} via API...${RESET_FORMAT}"
curl -X POST -H "Content-Type: application/json" \
  -H "Authorization: Bearer $token" \
...
echo "${BLUE_TEXT}${BOLD_TEXT}🏷️ Setting named ports for the instance group in REGION2: ${REGION2} via API...${RESET_FORMAT}"
curl -X POST -H "Content-Type: application/json" \
  -H "Authorization: Bearer $token" \
  -d '{"namedPorts": [{"name": "http", "port": 80}]}' \
...
echo "${BLUE_TEXT}${BOLD_TEXT}🚀 Creating the 'siege-vm' instance in ZONE3: ${ZONE3}...${RESET_FORMAT}"
echo "${RED_TEXT}${BOLD_TEXT}This will also configure Ops Agent and snapshot policies. ⚙️${RESET_FORMAT}"
...
echo "${BLUE_TEXT}${BOLD_TEXT}🛡️ Creating a new security policy 'rate-limit-siege' for rate limiting...${RESET_FORMAT}"
gcloud compute security-policies create rate-limit-siege \
    --description "policy for rate limiting"
echo

echo "${BLUE_TEXT}${BOLD_TEXT}🚦 Adding a rate limiting rule to 'rate-limit-siege' policy...${RESET_FORMAT}"
echo "${RED_TEXT}${BOLD_TEXT}This rule will ban IPs exceeding 50 requests in 120 seconds for 300 seconds. ✋${RESET_FORMAT}"
...
echo "${RED_TEXT}${BOLD_TEXT}🔗 Attaching the 'rate-limit-siege' security policy to the 'http-backend' service...${RESET_FORMAT}"
