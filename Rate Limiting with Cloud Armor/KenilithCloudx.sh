...
echo "${BLUE_TEXT}${BOLD_TEXT}Enabling the OS Config API for your project.${RESET_FORMAT}"
gcloud services enable osconfig.googleapis.com
echo

echo "${BLUE_TEXT}${BOLD_TEXT}Creating firewall rule to allow HTTP traffic...${RESET_FORMAT}"
...
echo "${BLUE_TEXT}${BOLD_TEXT}Creating firewall rule for health checks...${RESET_FORMAT}"
...
echo "${BLUE_TEXT}${BOLD_TEXT}Creating Managed Instance Group (MIG) and configuring autoscaling for REGION1: ${REGION1}${RESET_FORMAT}"
...
echo "${BLUE_TEXT}${BOLD_TEXT}Creating Managed Instance Group (MIG) and configuring autoscaling for REGION2: ${REGION2}${RESET_FORMAT}"
...
echo "${BLUE_TEXT}${BOLD_TEXT}Creating a default security policy for the backend service via API.${RESET_FORMAT}"
curl -X POST -H "Content-Type: application/json" \
  -H "Authorization: Bearer $token" \
  -d '{
...
echo "${BLUE_TEXT}${BOLD_TEXT}Creating the global backend service via API${RESET_FORMAT}"
curl -X POST -H "Content-Type: application/json" \
  -H "Authorization: Bearer $token" \
  -d '{
    "backends": [
...
echo "${BLUE_TEXT}${BOLD_TEXT}Creating the URL map for the load balancer via API${RESET_FORMAT}"
curl -X POST -H "Content-Type: application/json" \
  -H "Authorization: Bearer $token" \
  -d '{
...
echo "${BLUE_TEXT}${BOLD_TEXT}Creating the IPv4 Global Forwarding Rule via API${RESET_FORMAT}"
curl -X POST -H "Content-Type: application/json" \
  -H "Authorization: Bearer $token" \
  -d '{
    "IPProtocol": "TCP",
...
echo "${BLUE_TEXT}${BOLD_TEXT}Creating the IPv6 Global Forwarding Rule via API${RESET_FORMAT}"
curl -X POST -H "Content-Type: application/json" \
  -H "Authorization: Bearer $token" \
  -d '{
    "IPProtocol": "TCP",
...
echo "${BLUE_TEXT}${BOLD_TEXT}Setting named ports for the instance group in REGION1: ${REGION1} ${RESET_FORMAT}"
curl -X POST -H "Content-Type: application/json" \
  -H "Authorization: Bearer $token" \
...
echo "${BLUE_TEXT}${BOLD_TEXT}Setting named ports for the instance group in REGION2: ${REGION2} via API${RESET_FORMAT}"
curl -X POST -H "Content-Type: application/json" \
  -H "Authorization: Bearer $token" \
  -d '{"namedPorts": [{"name": "http", "port": 80}]}' \
...
echo "${RED_TEXT}${BOLD_TEXT}This will also configure Ops Agent and snapshot policies.${RESET_FORMAT}"
...
echo "${BLUE_TEXT}${BOLD_TEXT}Creating a new security policy 'rate-limit-siege' for rate limiting...${RESET_FORMAT}"
